...
//
// The Python's `cache/<kind>/<sha256(key)[:16]>.<ext>` on disk becomes one
// IndexedDB object store keyed by that same path string.
//
...
export class Cache {
   ...
   * ASYNC because `sha256Text` is; the one signature that differs from Python's `path_for`.
   *
