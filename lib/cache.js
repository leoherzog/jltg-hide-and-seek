// lib/cache.js — content-addressed HTTP cache (generate.py lines 495–548).
//
...
//     or the complete Overpass QL text with the bbox already interpolated — so
//     changing a border correctly invalidates.
...
// ── backing stores ───────────────────────────────────────────────────────────
//
// Two implementations behind one three-method interface. `indexedDB` does not
// exist in Node (the integration smoke test runs there) and can also be missing
// or throw in a locked-down browser profile, so the in-memory Map is not a test
// affordance — it is the real fallback. The public `Cache` API is identical
// either way; only `cache.backend` tells them apart.
...
class IdbStore {
  ...
  /** @param {string} name */
  async remove(name) {
    await request(this._tx('readwrite').delete(name));
  }
}
...
export class Cache {
   ...
   * smoke test wants.
   *
   * @param {{get:Function, put:Function, remove:Function, backend:string}} [store]
  ...
  constructor(store, opts = {}) {
    ...
    /** @type {string[]} entries served from the cache — the provenance block prints these */
    this.hits = [];
    /** @type {string[]} entries fetched and stored */
    this.misses = [];
  }
   ...
   *
   * @param {string} kind 'gtfs' | 'overpass' | 'nominatim'
  ...
  async put(kind, key, ext, body) {
    ...
    this.misses.push(name);
    return name;
  }

  /**
   * Drop an entry. Callers use this when a response parsed as garbage — a poisoned
   * cache entry would make every later run reproduce the failure. Mirrors the
   * Python's `cache.path_for(...).unlink(missing_ok=True)`; never throws.
   *
   * @param {string} kind @param {string} key @param {string} ext
   */
  async delete(kind, key, ext) {
    try {
      await this.store.remove(await this.nameFor(kind, key, ext));
    } catch {
      // A read-only cache is not our problem.
    }
  }
 ...
 *
 * @param {{offline?: boolean, refresh?: boolean, dbName?: string}} [opts]
...
/**
 * The 16-hex handle a request is filed under, without the `<kind>/` prefix or the
 * extension — the Python's `_cache_key16`, printed in §Provenance so a reader can
 * find the exact cached response behind a number. ASYNC.
 *
 * @param {string} text @returns {Promise<string>}
 */
export async function cacheKey16(text) {
