// lib/cache.js — content-addressed HTTP cache.
//
// Content is filed under one IndexedDB object store, keyed by the path string
// `cache/<kind>/<sha256(key)[:16]>.<ext>`.
//
//   * The key is the FULLY SUBSTITUTED request, so a different feed is a different
//     entry. Only 'gtfs' is stored; the world files are immutable and the browser's
//     HTTP cache handles them.
//   * Nothing time-derived is ever part of a key.
//   * The stored value is the response body only: no timestamp, headers or status.
...
// ── backing stores ───────────────────────────────────────────────────────────
//
// Two implementations behind `get` and `put`. There is no delete: `opts.refresh`
// makes `get` miss and the following write overwrites in place. `indexedDB` is
// absent in Node and can throw in a locked-down profile, so the in-memory Map is
// the real fallback, not a test affordance. Only `cache.backend` tells them apart.
...
// The backend the cache in force settled on. The scoring layer never sees the
// `Cache`, so this is how `buildProvenance` (rules/score.js) reports a silent
// fallback to the non-persistent Map. One run opens exactly one cache.
...
export class Cache {
  /**
   * Prefer `openCache()`, which picks the store. No store means in-memory,
   * which is what a Node smoke test wants.
   *
  ...
  /**
   * The `<kind>/<sha256(key)[:16]>.<ext>` handle a request is filed under.
   * ASYNC because `sha256Text` is.
   *
  ...
  async put(kind, key, ext, body) {
    ...
    try {
      ...
      // Quota exceeded or a private-mode store; losing the entry only costs a refetch.
    }
    return name;
  }
  ...
  requireOnline(what) {
    ...
    // The key rides on the error for diagnostics; the message a reader sees never prints it.
    const err = new CacheMiss('“Use only the cached transit feed” is on and this file is not in the cache. '
      + 'Turn it off to download it.');
    err.what = what;
    throw err;
  }
}
