...
//
// Ported from generate.py. Haversine, an equirectangular projection, a
// monotone-chain hull, Welzl, ray casting and segment distance, written out so
// no geometry library is needed and the determinism is readable.
//
// CONVENTIONS
...
function sortedUnique(points) {
  const seen = new Map();
  for (const p of points) {
    // `+0` folds -0 into 0, as a Python set does.
 ...
 *
 * The one intentional divergence from the CLI, which shuffles with Mersenne
 * Twister. The shuffle only gives Welzl its expected-linear time; the circle is
