...
//
// Haversine, an equirectangular projection, a monotone-chain hull, Welzl, ray
// casting and segment distance, written out so no geometry library is needed
// and the determinism is readable.
//
// CONVENTIONS
...
/**
 * Local equirectangular projection about a reference latitude: metres, x east /
 * y north, exactly reversible. Every distance, grid and area computation works
 * in this plane. Immutable; `{lat0, lon0}` is the structured-clone-safe wire form.
 */
export class Projection {
  ...
  /**
   * Planar → geographic. Returns `[lon, lat]`, not `[lat, lon]`.
 ...
 * Plain min/max, no antimeridian handling: a frame that straddles it is refused upstream.
 *
 ...
 * Touching along an edge counts, so callers wanting a real overlap should check
 * `bboxAreaSqM` of the result.
 *
...
/**
 * The ground distance between the nearest edges of two `[S, W, N, E]` boxes in
 * metres, 0 when they overlap or touch. The east–west gap is taken at the shared (or
 * facing) latitude nearest the equator, so it never overstates. Same per-degree
 * constants as `bboxAreaSqM`; no antimeridian handling, like `bboxUnion`.
 *
 * @param {[number, number, number, number]} a
 * @param {[number, number, number, number]} b
 * @returns {number} metres
 */
export function bboxGapM(a, b) {
  const dLat = Math.max(0, b[0] - a[2], a[0] - b[2]);
  const dLon = Math.max(0, b[1] - a[3], a[1] - b[3]);
  if (dLat === 0 && dLon === 0) return 0;
  const lo = Math.min(Math.max(a[0], b[0]), Math.min(a[2], b[2]));
  const hi = Math.max(Math.max(a[0], b[0]), Math.min(a[2], b[2]));
  const lat = lo <= 0 && hi >= 0 ? 0 : Math.min(Math.abs(lo), Math.abs(hi));
  const dy = dLat * 111132.0;
  const dx = dLon * 111320.0 * Math.max(0.05, Math.cos(lat * RAD));
  return Math.hypot(dx, dy);
}

/**
 * Group boxes that chain together within `maxGapM` of one another (single linkage:
 * A near B and B near C puts A and C together). Each group is its input indices,
 * ascending, and the groups are ordered by their first index. One group means one map.
 *
 * @param {Array<[number, number, number, number]>} bboxes
 * @param {number} maxGapM
 * @returns {number[][]}
 */
export function bboxChains(bboxes, maxGapM) {
  ...
  for (let i = 0; i < n; i++) {
    for (let j = i + 1; j < n; j++) {
      if (bboxGapM(bboxes[i], bboxes[j]) > maxGapM) continue;
      const ri = root(i);
      const rj = root(j);
  ...
  return Array.from(groups.values()).sort((x, y) => x[0] - y[0]);
}

/**
 * Area of an `[S, W, N, E]` box in square metres, on the same per-degree constants
 * `Projection` uses, so the landing page and `Border.areaSqM` agree. A degenerate
...
/**
 * Scale an `[S, W, N, E]` box about its centre (1.1 grows, 0.9 shrinks). Latitude
 * is clamped to the poles and longitude to ±180.
 *
 * @param {[number, number, number, number]} bbox
 * @param {number} factor
 * @returns {[number, number, number, number]}
...
/**
 * Do the planar segments `a→b` and `c→d` touch or cross? The standard orientation
 * test plus the four collinear-and-overlapping cases. Planar, so feeding it
 * `[lat, lon]` pairs is the same flat-earth assumption `pointInRing` makes.
 *
...
export function segIntersects(a, b, c, d) {
  // Only the sign is consulted; folding the magnitude away keeps the comparisons exact.
 ...
 *
 * Three cases, each needed: a box corner inside the ring (misses a box that
 * swallows the ring), a ring vertex inside the box (misses a small box deep inside
 * the ring), and a box edge crossing a ring edge. The cheap bbox reject runs first
 * because the caller tests ~1,400 boxes and almost all miss.
 *
 * `ring` is `[lat, lon]` pairs, first point NOT repeated. Touching counts.
 *
 * @param {[number, number, number, number]} bbox
...
/**
 * Dedupe planar [x, y] points on exact coordinate equality (with -0 folded to
 * 0), then sort lexicographically by x then y.
 * @param {Array<[number, number]>} points
...
/**
 * Shoelace **area** centroid of a planar ring; bbox centre when degenerate.
 * Not Overpass's `out center` (the bbox centre), which diverges by a measured
 * p90 of 88 m on real park polygons, enough to flip zone membership.
 * @param {Array<[number, number]>} ring
 ...
 * The area centroid when it is inside; for concave shapes, the midpoint of the
 * longest interior span on the horizontal line through the centroid.
 * @param {Array<[number, number]>} ring
 * @returns {[number, number]}
 */
...
/**
 * Length-weighted midpoint of an open planar polyline. Not the vertex mean,
 * which is biased toward dense clusters at curves.
 ...
 *
 * The *polygon* predicate, used by the rulebook's photo questions; matching and
 * measuring questions use the map icon instead. Measured spread: 45.5% of stops
 * by polygon vs 29.0% by icon. Never substitute one for the other.
 * @param {[number, number]} pt
 * @param {Array<[number, number]>} ring
 * @param {number} radius
 * @returns {boolean}
...
/**
 * mulberry32 — a 32-bit PRNG, used as a FIXED PERMUTATION, not as entropy.
 *
 * The shuffle only gives Welzl its expected-linear time; the circle is
 * the same under any permutation, within the 1e-7 slack of the `inside` test.
 * Seed 0, no clock, byte-identical across runs.
 * @param {number} seed
 ...
 * Input is sorted, deduped, then permuted by a fixed-seed Fisher-Yates
 * (see `mulberry32`).
 * @param {Array<[number, number]>} points
 ...
 * neighbourhood. Buckets keep insertion order and callers insert sorted, so
 * iteration is deterministic; `near()` returns `[key, x, y]` sorted by `String(key)`.
 *
 * Not structured-clone-safe (holds a Map): build inside the worker, flatten before emitting.
 */
export class GridIndex {
  ...
  /**
   * Insert an *area* feature into every cell its bbox touches. Returns false and
   * inserts nothing above `cap` cells; the caller linear-scans those few instead.
   * @param {*} key
