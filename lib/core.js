...
// ── the in-play stop set (CONTRACT.md §(b) Metrics, 2026-08-27) ─────────────
// A supplied border box or exclusion list narrows every measured quantity to the
// stops that survive it. Below this share of the day's served stops the filter is
// judged to have missed the city rather than trimmed it, and the whole served set
// is used instead (`Metrics.inPlayFallback`) — the same 50 % rule `inferBorder`
// applies to its own there-and-back test, so the two cannot disagree about what
// "kept most of the system" means.
export const IN_PLAY_MIN_SHARE = 0.5;

// ── the suggested border (gtfs/infer.js `suggestBorder`, 2026-08-27) ────────
// The worker never applies these; they decide whether a tighter, reachability-
// aware box is worth OFFERING on the page. Each is a named threshold so the
// prose can quote it and a reviewer can move one without reading the search.
...
/** A core must carry half the day's departures — departure-weighted so sparse outer stops cannot veto a dense core. */
export const SUGGEST_MIN_EVENT_SHARE = 0.5;
/** Degenerate-core floor, absolute: fewer stops than this is a neighbourhood, not a game. */
export const SUGGEST_MIN_CORE_STOPS = 100;
/** Degenerate-core floor, relative: a core under a tenth of the in-play set has missed the city. */
...
// The map's route-spoke layer (added 2026-08-23). The cap is applied WORKER-side,
// where the geometry is built, because the point of a cap on drawn polylines is that
// the bytes never cross `postMessage` — unlike `MAX_MAP_STOPS`, which drops a payload
// the page already has. It lives here because the worker caps and the page prints the
// cap sentence, and neither may own the number. 60 route-directions at 20 m of
// Ramer-Douglas-Peucker is ~1,400 points on the reference feed; a silent cap is
// forbidden, so `spokeCap.shown < spokeCap.total` is always said on the page.
...
// own it: the picker refuses the eleventh pick, `readSources` refuses an eleventh
// that arrived by a different door (ten map picks plus a dropped zip), and the worker
// refuses one it was handed anyway. The number is a memory and time bound — two
// metro feeds already merge 2.35 M stop_times — not a matter of taste. Raised from 6
// on 2026-08-27 so a metro split across borough-sized feeds (New York publishes its
// subway, five bus feeds and two ferries separately) fits in one run; the example
// maps in `lib/catalog.js` are the shape it was raised for.
...
/**
 * Code-point string order — Python's, and the only string comparator in the repo.
 *
 * `CONTRACT.md` §0 prescribes exactly this body: never `localeCompare`, which is
 * locale-dependent and therefore non-deterministic. It lives here because every
 * module already imports this one, and every one of them sorts strings: a single
 * implementation makes "the same order everywhere" true by construction rather
 * than by a file-per-copy agreeing. `tools/mdb-snapshot.mjs` keeps its own copy
 * on purpose — it is a standalone Node tool that imports nothing from the app.
 *
 * @param {string} a
 * @param {string} b
 * @returns {number}
 ...
 *
 * `sha256Text` is the right hash for a cache key or a feed's content address, and it
 * is the wrong one here: `crypto.subtle` is a Promise and it needs a secure context,
 * while the caller that needs this is `osmSourceRef`, which turns a drawn ring into a
 * `SourceRef.id` inside a click handler that cannot await and must not depend on the
 * page being served over https. So this is a second hash with a much smaller job:
 * give the same ring the same id across sessions, and different rings different ones.
 * It is NOT a content address and nothing may present it as one.
 *
 * Two independent 32-bit lanes — FNV-1a, and a rotate-and-mix over the same bytes
 * with the character's position folded in — concatenated. One FNV lane alone
 * collides on transpositions of equal-length inputs, which is exactly what two
 * polygons drawn round the same city look like. `charCodeAt` walks UTF-16 units,
 * which is deterministic for a given string and is all this needs.
 *
 * @param {string} text
