...
//
// Ported from generate.py's constants, number formatting and JSON/hashing sections.
//
...
export const DEGRADE_KIND = Object.freeze({
  ...
  path_join_not_evaluated: dk('Path test not evaluated', 'route', 'neutral', 'outlined', 'not_evaluated'),
  ...
  density_grid: dk('Grid counts: per-zone approximate', 'table-cells', 'neutral', 'outlined', 'limit'),
 ...
 * rounds the binary value (`(2.675).toFixed(2) === '2.67'`); this mirrors
 * Python's `Decimal(repr(x)).quantize(…, ROUND_HALF_UP)`.
 *
 * @param {number} x
 * @param {number} [dp=0]
 * @returns {number}
 ...
 * integer-like keys ('1','2','12') ahead of Python's lexicographic order.
 *
 * @param {*} obj
