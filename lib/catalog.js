...
import { bboxContains, bboxGapM, bboxIntersectsRing, ringCentroid } from './geo.js';
import { MAX_FEED_GAP_M, cmpStr, coord, num, stableHash } from './core.js';
...
/**
 * How far the row's box sits from the nearest of `boxes`, edge to edge, in km; 0
 * when it overlaps one or there are none.
 * @param {Object} row @param {Array<[number, number, number, number]>} boxes
 * @returns {number}
 */
export function gapKmOf(row, boxes) {
  let best = Infinity;
  for (const b of boxes || []) best = Math.min(best, bboxGapM(row.b, b));
  return Number.isFinite(best) ? best / 1000 : 0;
}

/**
 * True when the row is a second city next to `boxes` rather than more of the same
 * map: further than `MAX_FEED_GAP_M` from every one of them.
 * @param {Object} row @param {Array<[number, number, number, number]>} boxes
 * @returns {boolean}
 */
export function tooFarFrom(row, boxes) {
  return gapKmOf(row, boxes) * 1000 > MAX_FEED_GAP_M;
}
...
/** How far from the reader's position `rowsNear` looks, in km. */
export const NEAR_KM = 100;

/**
 * The rows around a point, nearest first: every box that contains it (gap 0),
 * then boxes within `withinKm` edge to edge. Ties go to the bigger network (`t`,
 * the same column `searchCatalog` ranks on) and then `id`, so a city's operator
 * lists above the campus shuttle inside it. This is what the landing's locate
 * button lists; the km is the same edge gap the results rows print for a too-far
 * pick, so the two figures agree.
 *
 * @param {Object[]} rows
 * @param {number} lat @param {number} lon
 * @param {{withinKm?: number, limit?: number}} [opts]
 * @returns {{rows: Object[], total: number}}
 */
export function rowsNear(rows, lat, lon, opts = {}) {
  const { withinKm = NEAR_KM, limit = 20 } = opts;
  if (!Number.isFinite(lat) || !Number.isFinite(lon)) return { rows: [], total: 0 };
  const here = [lat, lon, lat, lon];
  const scored = [];
  for (const row of rows) {
    const km = bboxContains(row.b, lat, lon) ? 0 : bboxGapM(row.b, here) / 1000;
    if (km <= withinKm) scored.push([km, row]);
  }
  scored.sort((a, b) => (a[0] - b[0]) || ((b[1].t || 0) - (a[1].t || 0)) || cmpStr(a[1].id, b[1].id));
  return { rows: scored.slice(0, limit).map((s) => s[1]), total: scored.length };
}
