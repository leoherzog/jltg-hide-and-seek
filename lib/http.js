...
/**
 * Delay for `seconds`. The only timing primitive in this module.
...
/**
 * Build the request URL. `params` is applied in insertion order.
...
export async function httpFetch(cache, opts) {
  ...
  for (const endpoint of endpoints) {
    for (let attempt = 1; attempt <= attemptsPerEndpoint; attempt++) {
      try {
        ...
        if (r.status === 429) {
          // A non-numeric (HTTP-date) Retry-After is treated as a failed attempt.
  ...
  if (kind === 'gtfs') {
    throw new Error(
      `The feed could not be downloaded from ${cacheKey}. ${detail.replace(/\.$/, '')}. `
      + 'Many transit agencies block downloads from a browser, and no setting on your side '
      + 'can change that. Download the .zip yourself and choose it with the file picker instead.',
    );
  }
